        ...
        <div class="col-sm-10 mx-auto">
          <table class="table table-responsive rounded border t-center p-4">
            <tbody>
              ...
              <tr>
                <td>
                  <a class="std_info" href="{{$assignment->assignment_file}}">{{$assignment->submitted_student}}</a>
                </td>
                <td>{{date('F j, Y (D), g:i a', strtotime($assignment->created_at))}}</td>
                ...
                <td>
                  <form action="{{route('give_marks')}}" method="POST">
                    ...
                    <input value="{{$assignment->topic}}" name="topic" hidden>
                    <input value="{{$assignment->submitted_student}}" name="std" hidden>
